...
const JoinLobbyGame = ({ gameInfo, setLobbyGames, lobbyGames }) => {
...
// timeOpened: 1588487347739
  let handleClick = () => {
    ...
    let player2Name, player2Wealth;
    if (userInfo.user) {
      player2Name = userInfo.user.userName;
      player2Wealth = userInfo.user.accumulatedWealth
    }
    else {
      player2Name = "no-account";
      player2Wealth = 0;
    }
    fetch('/be/lobby/join', {
      method: "POST",
      ...
      if (res.status === 200) {
        ...
        newGameInfo.Player2Wealth = userInfo.user.accumulatedWealth;
          ...
          newGameInfo.Player2 = "no-account";
          newGameInfo.Player2Wealth = 0;
        }
